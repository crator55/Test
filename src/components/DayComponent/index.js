...
const apiKey="a96ec5dda61242b8c3539ae929d421a5";
...
class DayComponent extends React.Component {
  ...
  handleSetEdit = reminder => {
   ...
   getWeather = async(payload,update)=>{
    

    const apiCall = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${payload.city}&cnt=16&appid=${apiKey}`);
    const response = await apiCall.json();  
    var forecast;
    response.list.forEach(element => {
      var apiDate = new Date(element.dt_txt);
      var dateString =apiDate.getFullYear()+'-'+('0' +(apiDate.getMonth()+1)).slice(-2)+'-'+('0' + apiDate.getDate()).slice(-2);     
  ...
  handleCreateUpdateReminder = (e, update) => {
      ...
      this.getWeather(payload,update);   
